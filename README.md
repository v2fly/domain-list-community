...
Attribute is useful for sub-group of domains, especially for filtering purpose. For example, the list of `google` domains may contain its main domains, as well as domains that serve ads. The ads domains may be marked by attribute `@ads`, and can be used as `geosite:google@ads` in V2Ray routing. Domains and services that originate from outside China mainland but are available in China mainland, may be marked by attribute `@cn`.
